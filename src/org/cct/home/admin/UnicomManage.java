...
public class UnicomManage extends Activity implements OnScrollListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		dialog = ProgressDialog.show(UnicomManage.this, "加载中", "加载中，请稍后..");
	...
	private class StreetManageThread extends Thread {
		...
		public void run() {
			...
			list_role = adminroleimpl.querybyroleid(2);

			if (size < firstload) {
				firstload = size;
			}
			list_area = adminimpl.querybyareaidandroleidbypage(2, areaid, 0,
					firstload);
	...
	class StreetManageHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			if (msg.what == 1) {
				...
				dialog.dismiss();
				Toast.makeText(UnicomManage.this, "未查询到任何记录",
	...
	private void loadMoreDate() {
		...
		isPerload = 0;
		adminimpl = new Adminimpl();
		list_area = adminimpl.querybyareaidandroleidbypage(2, areaid,
				count - 1, size);
		Adminroleimpl adminroleimpl = new Adminroleimpl();
		list_role = adminroleimpl.querybyroleid(2);
		k++;
