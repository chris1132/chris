...
public class UnicomSaleManage extends Activity implements OnScrollListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		dialog = ProgressDialog.show(UnicomSaleManage.this, "加载中", "加载中，请稍后..");
	...
	private class StreetManageThread extends Thread {
		...
		public void run() {
			...
			list_role = adminroleimpl.querybyroleid(1);

			if (size < firstload) {
				firstload = size;
			}
			list_area = adminimpl.querybyareaidandroleidbypage(1, areaid, 0,
					firstload);
			...
			listItemAdapter = new SimpleAdapter(UnicomSaleManage.this,
					listItem,// 数据源
	...
	class StreetManageHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			if (msg.what == 1) {
				...
				dialog.dismiss();
				Toast.makeText(UnicomSaleManage.this, "未查询到任何记录",
	...
	private void loadMoreDate() {
		...
		isPerload = 0;
		adminimpl = new Adminimpl();
		list_area = adminimpl.querybyareaidandroleidbypage(1, areaid,
				count - 1, size);
		Adminroleimpl adminroleimpl = new Adminroleimpl();
		list_role = adminroleimpl.querybyroleid(1);
		k++;
